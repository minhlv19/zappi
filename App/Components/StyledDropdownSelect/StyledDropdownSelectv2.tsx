...
const StyledDropdownSelectV2 = (props: StyledDropdownSelectProps): ReactElement => {
  const {
    options,
    placeholder,
    selectedIndex,
  ...
  const componentRef: React.LegacyRef<ModalDropdown<StyleDropdownRowData>> = useRef(null);
  useEffect(() => {
    if (componentRef && componentRef.current && selectedIndex) {
      componentRef.current.select(+selectedIndex || -1);
  ...
  const renderDropdownItem = (rowData: StyleDropdownRowData, index: string) => {
    if (_.includes(hideFields, index)) {
      return <></>;
    }
...
const styles = StyleSheet.create({
  ...
  dropdownStyle: {
    ...
    borderWidth: 1,
    marginTop: Platform.OS === 'ios' ? rh(6) : rh(0),
  },
  dropdown: {
    width: rw(75),
    height: rh(42),
...
export default StyledDropdownSelectV2;
